...
export default function PharmacistRegister() {
    ...
    const [form, setForm] = useState({
        ...
        cin_numero: '', numero_autorisation: '', nom_pharmacie: '', gouvernorat: 'Tunis',
        adresse: '', matricule_fiscal: ''
    });
    ...
    const [docAutorisation, setDocAutorisation] = useState<File | null>(null);
    const [docPatente, setDocPatente] = useState<File | null>(null);
    ...
    // ── Navigation STEP 3 → 4 ─────────────────────────────
    const goStep4 = () => {
        if (!form.cin_numero || !form.numero_autorisation || !form.nom_pharmacie || !form.gouvernorat || !form.adresse || !form.matricule_fiscal) {
            toast('Veuillez remplir tous les champs de la pharmacie.', 'error'); return;
        }
    ...
    // ── Navigation STEP 4 → 5 ─────────────────────────────
    const goStep5 = () => {
        if (!docCIN || !docAutorisation || !docPatente) { toast('Veuillez uploader tous les documents requis.', 'error'); return; }
        setStep(5);
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (docAutorisation) fd.append('document_autorisation', docAutorisation);
            if (docPatente) fd.append('document_patente', docPatente);
                ...
                <div style={{ background: 'var(--bg-card)', borderRadius: 'var(--radius-lg)', padding: '28px', boxShadow: 'var(--shadow-sm)', border: '1px solid var(--border)' }}>
                    ...
                    {step === 1 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                                <div>
                                    <label style={labelStyle}>Prénom du pharmacien</label>
                                    <input style={inputStyle} placeholder="Mohamed" value={form.prenom} onChange={e => set('prenom', e.target.value)} />
                                </div>
                                <div>
                                    <label style={labelStyle}>Nom du pharmacien</label>
                                    <input style={inputStyle} placeholder="Trabelsi" value={form.nom} onChange={e => set('nom', e.target.value)} />
                                </div>
                            </div>

                            <div>
                                <label style={labelStyle}>Email professionnel</label>
                                <input style={inputStyle} type="email" placeholder="pharmacie.alamal@gmail.com" value={form.email} onChange={e => set('email', e.target.value)} />
                            </div>
                            <div>
                    ...
                    {step === 3 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                            ...
                            <div>
                                <label style={labelStyle}>Numéro Autorisation <span style={{ fontWeight: '400', color: 'var(--text-muted)' }}>(Délivré par le Ministère)</span></label>
                                <input style={inputStyle} placeholder="N° Autorisation" value={form.numero_autorisation} onChange={e => set('numero_autorisation', e.target.value)} />
                            </div>
                            <div>
                                <label style={labelStyle}>Nom commercial pharmacie</label>
                                <input style={inputStyle} placeholder="Ex: Pharmacie Al Amal" value={form.nom_pharmacie} onChange={e => set('nom_pharmacie', e.target.value)} />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <input style={inputStyle} placeholder="Rue, numéro, ville" value={form.adresse} onChange={e => set('adresse', e.target.value)} />
                            </div>
                            <div>
                                <label style={labelStyle}>Matricule fiscal</label>
                                <input style={inputStyle} placeholder="Numéro fiscal de la pharmacie" value={form.matricule_fiscal} onChange={e => set('matricule_fiscal', e.target.value)} />
                            </div>
                    ...
                    {step === 4 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            ...
                            <p style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Format accepté : PDF ou JPG — Taille maximale : 5 MB par fichier</p>

                            {([
                                { label: 'CIN du pharmacien', file: docCIN, set: setDocCIN, accept: '.pdf,.jpg,.jpeg', required: true },
                                { label: "Autorisation d'ouverture", file: docAutorisation, set: setDocAutorisation, accept: '.pdf,.jpg,.jpeg', required: true },
                                { label: 'Patente commerciale', file: docPatente, set: setDocPatente, accept: '.pdf,.jpg,.jpeg', required: true },
                    ...
                    {step === 5 && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            <h2 style={{ fontWeight: '700', fontSize: '18px', marginBottom: '4px' }}>Contrat & Acceptation</h2>
                            
                            <div style={{ padding: '12px', background: 'var(--blue-50)', borderRadius: 'var(--radius-md)', border: '1px solid var(--blue-200)', marginBottom: '8px' }}>
                                <h3 style={{ fontSize: '13px', fontWeight: '700', color: 'var(--blue-800)', marginBottom: '6px' }}>Rappel des conditions financières</h3>
                                <ul style={{ fontSize: '13px', color: 'var(--blue-700)', paddingLeft: '16px', margin: 0 }}>
                                    <li>Bonus par consultation : <strong style={{color:'var(--blue-900)'}}>[X] DT</strong></li>
                                    <li>Abonnement mensuel : <strong style={{color:'var(--blue-900)'}}>[X] DT/mois</strong></li>
                                </ul>
                            </div>
                            ...
                            {[
                                ...
                                <span key="contrat">J'ai lu et j'accepte le → <a href="/legal/contrat-medecin.html" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--blue-600)', textDecoration: 'underline' }} onClick={e => e.stopPropagation()}>Contrat de Partenariat Pharmacie-Plateforme — Version 1.0</a></span>,
                                "J'accepte les modalités financières : bonus [X] DT par consultation et abonnement [X] DT/mois",
                                ...
                                "Je certifie que mes documents et autorisations officielles sont en règle et à jour",
                            ].map((text, i) => (
